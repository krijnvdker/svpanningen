	...
	*/	
	...
	function gdlr_check_player_item( $type, $settings = array() ){
		if($type == 'player'){
			...
			gdlr_print_top_scorer_item( $settings );
		}
	}
	
	// top scorer item
	function gdlr_print_top_scorer_item( $settings ){
		...
		echo '<div class="top-scorer-item-wrapper gdlr-item" ' . $item_id . $margin_style . '" >'; 		
		...
		while($query->have_posts()){ $query->the_post();
			...
			echo '<div class="top-scorer-item-list" >';
			echo gdlr_soccer_get_player_avatar('thumbnail');

			echo '<div class="top-scorer-item-content-wrap" >'; 
			echo '<h3 class="top-scorer-item-title" >' . get_the_title() . '</h3>';

			$goals = get_post_meta(get_the_ID(), 'gdlr-soccer-player-goals', true);
			if( $goals > 1 ){
				echo '<div class="top-scorer-item-goals" >' . $goals . ' ' . esc_html__('Goals', 'gdlr-soccer') . '</div>';
			}else{
				echo '<div class="top-scorer-item-goals" >' . $goals . ' ' . esc_html__('Goal', 'gdlr-soccer') . '</div>';
			}
			echo '</div>';
		...
		echo '</div>'; // top scorer item wrapper
	}
