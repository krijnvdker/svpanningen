	...
	*/	
		...
		$player_settings = array(
			...
			'player-stats' => array(
				...
				'options' => array(
					...
					'enable-player-stats' => array(
						'title' => __('Enable Player Stats Tab', 'gdlr-soccer'),
						'type' => 'checkbox',
						'default' => apply_filters('realsoccer_enable_player_stats', 'enable')
					),
					'passes' => array(
	...
	function gdlr_soccer_save_player_meta_box($post_id){
		...
		if( isset($_POST['gdlr-soccer-player-settings']) ){
			$player_val = $_POST['gdlr-soccer-player-settings'];
			$player_options = empty($player_val)? array(): json_decode(stripslashes($player_val), true);
			update_post_meta($post_id, 'gdlr-soccer-player-settings', gdlr_lms_preventslashes($player_val));

			if( !empty($player_options['player-stats']['goals']) ){
				update_post_meta($post_id, 'gdlr-soccer-player-goals', $player_options['player-stats']['goals']);
			}
		}
	...
	function gdlr_register_player_item( $page_builder = array() ){
		...
		$page_builder['content-item']['options']['top-scorer'] = array(
			'title'=> __('Top Scorer Widget', 'gdlr-soccer'), 
			'type'=>'item',
			'options'=>array_merge(gdlr_page_builder_title_option(true), array(					
				'category'=> array(
					'title'=> __('Category' ,'gdlr-soccer'),
