<?php
	...
	*/	
	...
	function gdlr_register_fixture_result_item( $page_builder = array() ){
		...
		$page_builder['content-item']['options']['recent-match'] = array(
			'title'=> __('Recent Match', 'gdlr-soccer'), 
			'type'=>'item',
			'options'=>array(		
				'category'=> array(
					'title'=> __('Category' ,'gdlr-soccer'),
		...
		$page_builder['content-item']['options']['fixture-result'] = array(
			...
			'options'=>array_merge(gdlr_page_builder_title_option(true), array(					
				...
				'year-filter'=> array(
					'title'=> __('Enable Year Filter' ,'gdlr-soccer'),
					'type'=> 'checkbox',
					'default'=> 'disable',
					'wrapper-class'=> 'style-wrapper full-wrapper'
				),						
				'order'=> array(
