...
input_hidden.style.width = '60px';
    ...
    document.getElementById('playerzSub_id').value =  0;
    //jQuery('#playerzSub_id_chosen').find('.search-choice').remove();
    
    jQuery('#playerzSub_id').trigger("chosen:updated");
...
function getSubEvents(){
    ...
    jQuery("#ncPlSubTitle").html("");
    jQuery("#ncPlSub").hide();
    jQuery("#subeventid").val("0");
    console.log(eventid);
    var data = {
        'action': 'getsubsevent',
        'event_id': eventid,
        ...
        jQuery.post(ajaxurl, data, function(res) {
           ...
           if(res && res != 'null'){
                ...
                if(jsOn.name && jsOn.id){
                    jQuery("#ncPlSubTitle").html(jsOn.name);
                    jQuery("#ncPlSub").show();
