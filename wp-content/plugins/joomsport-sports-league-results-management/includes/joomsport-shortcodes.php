  ...
  if(count($list)){
          ...
          public static function joomsport_playerlist($attr){
            ...
            if($ttype){
                   ...
                   if($row->ID){
                        ...
                        for ($intB = 0; $intB < count($ef); ++$intB) {
                            ...
                            if(isset($ef_type) && $ef_type == 3){
                                $orderValue = -1;
                                $meta = get_post_meta($row->ID,'_joomsport_player_ef',true);
                                $meta_s = get_post_meta($row->ID,'_joomsport_player_ef_'.$args['season_id'],true);
                ...
                $rows->lists['events_col'] = classJsportgetplayers::getPlayersEvents($args['season_id']);
