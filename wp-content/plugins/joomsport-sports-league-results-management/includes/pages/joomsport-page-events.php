    ...
    public static function view(){
        ...
        if (isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
            ...
            if(isset($_POST['dependson']) && count($_POST['dependson'])){
                $item['dependson'] = json_encode($_POST['dependson']);
            }
            if ($item_valid === true) {
                if ($item['id'] == 0) {
                    ...
                    $wpdb->query("DELETE FROM {$wpdb->joomsport_events_depending} WHERE subevent_id = ".intval($item['id']));
                    $dependson = $_POST["dependson"];
                    if(count($dependson)){
                        foreach($dependson as $dp){
                            $query = "INSERT INTO {$wpdb->joomsport_events_depending}(subevent_id,event_id)"
                                    . " VALUES(".intval($item['id']).",".intval($dp).")";
                            $wpdb->query($query);
  ...
  delImgLink.on( 'click', function( event ){
			...
			<tr>
				...
				<td colspan="2">
					<table cellpadding="0" id="calctp_es" <?php echo ($item['player_event'] == 1) ? '' : "style='display:none;'";?>>
                        ...
                        <tr>
                            <td class="hideFromMatchEv" <?php echo ($item['player_event'] == 1) ? '' : "style='display:none;'";?>>
                                <?php echo __('Related to', 'joomsport-sports-league-results-management'); ?>
    ...
    public static function getListValues($item){
        ...
        $query = "SELECT e.e_name as name,e.id"
                ...
                . " WHERE de.subevent_id = ".intval($item["id"]).""
                . " ORDER BY e.e_name";
        $evnsSelected = $wpdb->get_results($query);
        ...
        $query = "SELECT e.e_name as name,e.id"
                ...
                . " AND de.id IS NULL AND e.id != ".intval($item["id"]).""
                . " ORDER BY e.e_name";
        $evns = $wpdb->get_results($query);
        ...
        $html = '';
        if(count($evns)){
            $html .=  '<select name="dependson[]" class="jswf-chosen-select" data-placeholder="'.__('Add item','joomsport-sports-league-results-management').'" multiple>';
            foreach ($evns as $tm) {
                $selected = '';
                if(in_array($tm->id, $dependson)){
                    $selected = ' selected';
                }
                $html .= '<option value="'.$tm->id.'" '.$selected.'>'.$tm->name.'</option>';
            }
            $html .= '</select>';
        }else{
