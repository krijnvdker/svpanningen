    ...
    public static function joomsport_player_seasonrelated(){
        ...
        if($season_id && $post_id){
            ...
            for($intA=0;$intA<count($teamsSelected);$intA++){
                ...
                $jerseyHTML .= '<div class="jstable-cell"><input type="number" id="jersey_'.$teamsSelected[$intA].'" name="jersey['.$teamsSelected[$intA].']" step=1 value="'.(isset($jersey[$teamsSelected[$intA]])?$jersey[$teamsSelected[$intA]]:'').'" /></div>';
                $jerseyHTML .= '</div>';
            }
            $jerseyHTML .= '</div>';
