    ...
    public function save(){
        ...
        if(isset($fdata['match_id']) && count($fdata['match_id'])){
            for($intA = 0; $intA < count($fdata['match_id']); $intA++){
                ...
                $metadata = get_post_meta(intval($fdata['home_team'][$intA]),'_joomsport_team_personal',true);
                $home_team = isset($metadata['middle_name'])?esc_attr($metadata['middle_name']):"";
                
                $metadata = get_post_meta(intval($fdata['away_team'][$intA]),'_joomsport_team_personal',true);
                $away_team = isset($metadata['middle_name'])?esc_attr($metadata['middle_name']):"";
                if(!$home_team){
