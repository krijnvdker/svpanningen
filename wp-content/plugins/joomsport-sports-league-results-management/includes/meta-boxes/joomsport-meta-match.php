...
class JoomSportMetaMatch {
    ...
    public static function js_meta_playerevents($post){
        ...
        $query = "SELECT me.*,ev.e_name,(subev.e_id) as subevID, GROUP_CONCAT(CONCAT(subev.t_id,'*',subev.player_id)) as subevPl, GROUP_CONCAT(subev.player_id) as plFM"
                ...
                . " JOIN {$wpdb->joomsport_events} as ev ON me.e_id = ev.id AND me.match_id = ".$post->ID
        ...
        <table class="table table-striped jsTblEvnts"  cellspacing="0" cellpadding="0">
            ...
            if (isset($pevents) && $pevents) {
                foreach ($pevents as $m_events) {
                    ...
                    echo '<td>'.$assist_players.'<input type="hidden" name="sub_eventid[]" value="'.$m_events->subevID.'" /><input type="hidden" name="sub_eventid_vals[]" value="'.$m_events->subevPl.'" /></td>';
            ...
            <tfoot>
                <tr>
                    ...
                    <td>
                        <input type="hidden" id="subeventid" name="subeventid" value="0" />
                          <div id="ncPlSubTitle"></div>                                                                                       
                    </td>
                    <td class="title" width="60">
                    </td>

                    <td style="display: none;" class="title" width="60">
                ...
                <tr>
                    ...
                    <td class="title" width="200">
                        <?php echo JoomSportHelperSelectBox::Simple('event_id', $events,0,' id="event_id" onchange="getSubEvents();"',true);?>
                    </td>
                    <td>
                    ...
                    <td>
                        <div id="ncPlSub" style="display:none;">
                            <select id="playerzSub_id" name="playerzSub_id[]" multiple class="jswf-chosen-select" size="1" data-placeholder="Select player">
                            <?php
                            if($t_single){
                                echo '<option value="'.$home_team.'">'.get_the_title($home_team).'</option>';
                                echo '<option value="'.$away_team.'">'.get_the_title($away_team).'</option>';        
    ...
    private static function saveMetaPlayerEvents($post_id){
        ...
        if(isset($_POST['em_id']) && count($_POST['em_id'])){
            for($intA=0;$intA<count($_POST['em_id']);$intA++){
                ...
                //subevents
                if(isset($_POST['sub_eventid_vals'][$intA]) && $_POST['sub_eventid_vals'][$intA]){
                    $plisSubArr = explode(',', $_POST['sub_eventid_vals'][$intA]);
                    $eorderingSub = 0;
                    if(count($plisSubArr)){
                        foreach($plisSubArr as $plisSub){
                            $plis = explode('*', $plisSub);
                            /*$query = 'INSERT INTO #__bl_match_events(e_id,player_id,match_id,ecount,minutes,t_id,eordering,additional_to) VALUES('.intval($_POST['sub_eventid'][$i]).','.intval($plis[0]).','.$row->id.','.intval($_POST['e_countval'][$i]).','.addslashes($_POST['e_minuteval'][$i]).','.intval($plis[1]).','.$eorderingSub.','.$curev.')';
                            ...
                            $wpdb->insert($wpdb->joomsport_match_events, 
                                    array(
                                        "e_id" => intval($_POST['sub_eventid'][$intA]),
                                        "match_id" => $post_id,
