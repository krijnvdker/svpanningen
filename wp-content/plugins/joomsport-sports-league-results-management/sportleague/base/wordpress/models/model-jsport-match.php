...
class modelJsportMatch
{
    ...
    public function getPlayerEvents()
    {
        ...
        if($season_id){
            ...
            $query = "SELECT me.*,ev.*,(subev.e_id) as subevID, GROUP_CONCAT(CONCAT(subev.player_id,'*',subev.t_id)) as subevPl,evSub.e_name as subEn, GROUP_CONCAT(subev.player_id) as plFM"
                    ." FROM  {$wpdb->joomsport_match_events} as me"
                    . " JOIN {$wpdb->joomsport_events} as ev ON me.e_id = ev.id AND me.match_id = ".$this->match_id
                ...
                $query = "SELECT me.*,ev.*,(subev.e_id) as subevID, GROUP_CONCAT(CONCAT(subev.player_id,'*',subev.t_id)) as subevPl,evSub.e_name as subEn, GROUP_CONCAT(subev.player_id) as plFM"
                    ...
                    . " LEFT JOIN {$wpdb->joomsport_events} as evSub ON subev.e_id = evSub.id "
                       ...
                       ." WHERE ev.player_event = '1' AND ev.dependson='' AND (("
                        .($single ? 'me.player_id='.intval($home_team) : 'me.t_id='.intval($home_team))
                        .' AND me.e_id NOT IN ('.implode(',', $opposite_events).')) OR (me.e_id IN ('.implode(',', $opposite_events).') AND '
                        .($single ? 'me.player_id='.intval($away_team) : 'me.t_id='.intval($away_team))
                        .')) GROUP BY me.id'        
