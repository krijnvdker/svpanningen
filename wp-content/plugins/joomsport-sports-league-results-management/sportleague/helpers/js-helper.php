...
class jsHelper
{
    ...
    public static function getScore($match, $class = '', $tooltip = '', $itemid = 0)
    {
        ...
        $htmlLive = '';
        if($m_played == -1){
            $htmlLive = '<div class="jscalendarLive">'.__("Live", 'joomsport-sports-league-results-management' ).'</div>';
        }
    ...
    public static function getScoreBigM($match)
    {
        ...
        $htmlLive = '';
        if($m_played == -1){
            echo '<input type="hidden" id="match_id" value="'.$match->id.'" /><script>js_live_interval_guest = setInterval(jsLiveCheckUpdts, 30000);</script>';
            $htmlLive = '<div class="jscalendarLiveBig">'.__("Live", 'joomsport-sports-league-results-management' ).'</div>';
        }
